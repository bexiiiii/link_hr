---
name: Link HR
description: iOS HR app for companies in Kazakhstan. A quiet white interface, clear blue actions, Phosphor icons and a labelled bottom navigation.
colors:
  ground: "#F5F8FC"
  surface: "#FFFFFF"
  surface-alt: "#F0F5FB"
...
spacing:
  ...
  section-gap: 28px
components:
  nav-bar: "floating white pill, 16px from the bottom edge, 5 labelled icon buttons, active = blue icon + label only, soft shadow"
...
- Нижняя панель — белая плавающая панель в 16px от нижнего края: Главная, Посещаемость, Задачи, Сервисы, Профиль. У каждого пункта есть подпись. Активный пункт выделяется только синим контурным значком и подписью, без большого цветного круга. Неактивные иконки нейтральные.
