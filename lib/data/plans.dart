...
class DayPlan {
  DayPlan({required this.name, required this.date, required this.value, required this.comment});
...
abstract final class Plans {
  ...
  static Future<void> save(DateTime day, int value, String comment, {String? existing}) async {
    final html = '<p>$metric: $value</p>${comment.trim().isEmpty ? '' : '<p>${escapeHtml(comment.trim())}</p>'}';
    if (existing != null) {
