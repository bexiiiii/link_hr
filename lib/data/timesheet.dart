...
class DayRecord {
  DayRecord({required this.date, required this.mark, this.firstIn, this.lastOut, this.lateMinutes = 0, this.hours = 0});
...
class MonthSheet {
  ...
  int get present => count(DayMark.onTime) + count(DayMark.late) + count(DayMark.remote);
  ...
  Duration? get averageArrival {
    final arrivals = days.where((d) => d.firstIn != null).map((d) => d.firstIn!).toList();
    if (arrivals.isEmpty) return null;
    final minutes = arrivals.fold<int>(0, (s, t) => s + t.hour * 60 + t.minute) ~/ arrivals.length;
    return Duration(minutes: minutes);
  }

  String get workStartLabel => hhmm(workStart);

  static String hhmm((int, int) t) => '${t.$1.toString().padLeft(2, '0')}:${t.$2.toString().padLeft(2, '0')}';

  static Future<MonthSheet> load(DateTime month) async => (await loadRange(month, month)).first;

  /// Loads several consecutive months with three requests in total.
  static Future<List<MonthSheet>> loadRange(DateTime firstMonth, DateTime lastMonth) async {
    ...
    final results = await Future.wait<Object>([
      ...
      Hr.checkinsBetween(from, DateTime(to.year, to.month, to.day + 1)).catchError((_) => <Json>[]),
    ...
    return [
      for (var m = from; !m.isAfter(lastMonth); m = DateTime(m.year, m.month + 1)) build(m, events, logs, start),
    ];
  }

  static MonthSheet build(DateTime month, Map<String, String> events, List<Json> logs, (int, int) start,
    ...
    for (final list in byDay.values) {
      list.sort((a, b) => Fmt.parse(a['time'])!.compareTo(Fmt.parse(b['time'])!));
    }
    ...
    for (var d = from; !d.isAfter(to); d = DateTime(d.year, d.month, d.day + 1)) {
      ...
      final ins = dayLogs.where((l) => l['log_type'] == 'IN').map((l) => Fmt.parse(l['time'])!).toList();
      final outs = dayLogs.where((l) => l['log_type'] == 'OUT').map((l) => Fmt.parse(l['time'])!).toList();
      ...
      days.add(DayRecord(date: d, mark: mark, firstIn: firstIn, lastOut: lastOut, lateMinutes: late, hours: hours));
    }
    return MonthSheet(from, days, start);
  }

  static (int, int) workStartOf(List<Json> shifts) => _shiftTime(shifts, 'start_time', (9, 0));

  static (int, int) workEndOf(List<Json> shifts) => _shiftTime(shifts, 'end_time', (18, 0));

  static (int, int) _shiftTime(List<Json> shifts, String field, (int, int) fallback) {
    final today = Fmt.dateOnly(DateTime.now());
    for (final s in shifts) {
      ...
      if (parts.length >= 2) return (int.tryParse(parts[0]) ?? fallback.$1, int.tryParse(parts[1]) ?? fallback.$2);
    }
    return fallback;
  }
