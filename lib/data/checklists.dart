...
class ChecklistMeta {
  ...
  static ChecklistMeta parse(String? html) {
    ...
    return ChecklistMeta(
      title: rest.isEmpty ? 'Чеклист' : rest.first,
      from: from,
      to: to,
      photo: photo,
    );
...
abstract final class Checklists {
  ...
  static Future<List<ChecklistTemplate>> templates({
    bool mineOnly = false,
  }) async {
    ...
    final items = await _children(
      rows.map((r) => r['name'].toString()).toList(),
    );
    return [
      for (final r in rows)
        ChecklistTemplate(
  ...
  static Future<void> createTemplate({
    ...
    for (final user
        in assignees.isEmpty ? [Session.instance.userId] : assignees) {
  ...
  static Future<void> archiveTemplate(String name) =>
      _api.setValue('ToDo', name, {'status': 'Cancelled'});
  ...
  static Future<List<ChecklistRun>> today() async {
    ...
    final existing = {
      for (final r in results[1]) r['reference_name'].toString(),
    };
    final missing = templates
        .where((t) => !existing.contains(t['name'].toString()))
        .toList();
    if (missing.isNotEmpty) {
      final templateItems = await _children(
        missing.map((t) => t['name'].toString()).toList(),
      );
      for (final t in missing) {
        ...
        for (final item in templateItems[t['name']] ?? const <Json>[]) {
          await _insertItem(
            name,
            stripHtml(item['description']?.toString()),
            me,
          );
        }
      }
  ...
  static Future<List<ChecklistRun>> runs({
  ...
  static Future<void> setItem(ChecklistItem item, ItemState state) =>
      _api.setValue('ToDo', item.name, {
        'status': switch (state) {
  ...
  static Future<void> complete(ChecklistRun run) =>
      _api.setValue('ToDo', run.name, {'status': 'Closed'});

  static ChecklistRun _run(
    Json r,
    List<Json> items, [
  ...
  static Future<List<Json>> _runRows({
    required DateTime from,
    required DateTime to,
    String? user,
  }) => _api.list(
  ...
  static Future<void> _insertItem(String parent, String title, String user) =>
      _api.insert({
        'doctype': 'ToDo',
