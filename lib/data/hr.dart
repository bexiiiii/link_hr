...
abstract final class Hr {
  ...
  static List<Json> _rows(dynamic v) => (v as List? ?? const [])
      .map((e) => (e as Map).cast<String, dynamic>())
      .toList();

  // Attendance ------------------------------------------------------------

  static Future<List<Json>> checkins({int limit = 20, int start = 0}) =>
      _api.list(
        'Employee Checkin',
        fields: [
          'name',
  ...
  static Future<List<Json>> checkinsBetween(DateTime from, DateTime to) =>
      _api.list(
        'Employee Checkin',
  ...
  static Future<String> attachCheckinPhoto(
    ...
    if (url.isEmpty)
      throw Exception('Сервер не сохранил фото. Попробуйте ещё раз.');
    return url;
  }
  ...
  static Future<Map<String, String>> checkinPhotos(List<String> names) async {
    ...
    return {
      for (final r in rows)
        r['attached_to_name'].toString(): r['file_url'].toString(),
    };
  }
    ...
    return _api.list(
      ...
      filters: [
        ...
        [
          'time',
          'between',
          [Fmt.iso(now), Fmt.iso(DateTime(now.year, now.month, now.day + 1))],
        ],
      ],
      orderBy: 'time desc',
      limit: 20,
  ...
  static Future<Map<String, String>> calendarEvents(
    ...
    return (data as Map? ?? const {}).map(
      (k, v) => MapEntry(k.toString(), v.toString()),
    );
  }

  static Future<List<Json>> holidays() async => _rows(
    await _api.call('hrms.api.get_holidays_for_employee', {
      'employee': _s.employeeId,
    }),
  );

  static Future<List<Json>> shiftAssignments() async =>
      _rows(await _api.call('hrms.api.get_shifts'));

  static Future<List<Json>> shiftTypes() => _api.list(
    'Shift Type',
  ...
  static Future<List<Json>> shiftApprovers() async => _rows(
    await _api.call('hrms.api.get_shift_request_approvers', {
      'employee': _s.employeeId,
    }),
  );
  ...
  static Map<String, dynamic> _requestArgs(
    bool team,
    int? limit, {
    bool withApprover = true,
  }) => {
    'employee': _s.employeeId,
  ...
  static Future<List<Json>> leaves({bool team = false, int? limit}) async =>
      _rows(
        await _api.call(
          ...
          _requestArgs(team, limit),
        ),
      );

  static Future<List<Json>> expenseClaims({
    bool team = false,
    int? limit,
  }) async => _rows(
    await _api.call('hrms.api.get_expense_claims', _requestArgs(team, limit)),
  );

  static Future<List<Json>> shiftRequests({
    bool team = false,
    int? limit,
  }) async => _rows(
    await _api.call('hrms.api.get_shift_requests', _requestArgs(team, limit)),
  );

  static Future<List<Json>> attendanceRequests({
    bool team = false,
    int? limit,
  }) async => _rows(
    await _api.call(
      'hrms.api.get_attendance_requests',
      _requestArgs(team, limit, withApprover: false),
    ),
  );
  ...
  static Future<List<Json>> advanceBalance() async =>
      _rows(await _api.call('hrms.api.get_employee_advance_balance'));
  ...
  static Future<List<String>> leaveTypes(DateTime date) async {
    final data = await _api.call('hrms.api.get_leave_types', {
      'employee': _s.employeeId,
      'date': Fmt.iso(date),
    });
  ...
  static Future<num> leaveBalanceOn({
    required String leaveType,
    required DateTime from,
    required DateTime to,
  }) async {
  ...
  static Future<Json> leaveApprovalDetails() async =>
      ((await _api.call('hrms.api.get_leave_approval_details', {
                    'employee': _s.employeeId,
                  }))
                  as Map? ??
              {})
  ...
  static Future<Json> expenseSummary() async =>
      ((await _api.call('hrms.api.get_expense_claim_summary')) as Map? ?? {})
          .cast<String, dynamic>();

  static Future<List<Json>> expenseTypes() async =>
      _rows(await _api.call('hrms.api.get_expense_claim_types'));

  static Future<Json> expenseApprovalDetails() async =>
      ((await _api.call('hrms.api.get_expense_approval_details', {
                    'employee': _s.employeeId,
                  }))
                  as Map? ??
              {})
  ...
  static Future<Json> companyCostCenter() async =>
      ((await _api.call(
  ...
  static Future<List<Json>> unclaimedAdvances() async => _rows(
    await _api.call(
  ...
  static Future<String?> advanceAccount() async => (await _api.call(
    'hrms.api.get_advance_account',
    {'company': _s.company},
  ))?.toString();

  static Future<List<Json>> modesOfPayment() => _api.list(
    'Mode of Payment',
    fields: ['name', 'type'],
    filters: {'enabled': 1},
    limit: 100,
  ...
  static Future<List<Json>> salarySlips({DateTime? from, DateTime? to}) =>
      _api.list(
        'Salary Slip',
        fields: [
          'name',
  ...
  static Future<String> salarySlipPdf(String name) async => (await _api.call(
    'hrms.api.download_salary_slip',
    {'name': name},
  )).toString();

  // Notifications ---------------------------------------------------------

  static Future<int> unreadNotifications() async => Fmt.number(
    await _api.call('hrms.api.get_unread_notifications_count'),
  ).toInt();

  static Future<List<Json>> notifications({int start = 0, int limit = 20}) =>
      _api.list(
        'PWA Notification',
        fields: [
          'name',
  ...
  static Future<void> markAllNotificationsRead() =>
      _api.call('hrms.api.mark_all_notifications_as_read');

  // Workflow & permissions ------------------------------------------------

  static Future<Json> workflow(String doctype) async =>
      ((await _api.call('hrms.api.get_workflow', {'doctype': doctype}))
                  as Map? ??
              {})
          .cast<String, dynamic>();

  static Future<List<String>> workflowTransitions(Json doc) async {
    final data = await _api.call('frappe.model.workflow.get_transitions', {
      'doc': doc,
    });
  ...
  static Future<void> applyWorkflow(Json doc, String action) => _api.call(
    'frappe.model.workflow.apply_workflow',
    {'doc': doc, 'action': action},
  );

  static Future<Map<String, bool>> docPermissions(
    String doctype,
    String name,
  ) async {
    final data = await _api.call('frappe.client.get_doc_permissions', {
      'doctype': doctype,
      'docname': name,
    });
  ...
  static Future<List<String>> writableFields(String doctype) async {
    final data = await _api.call('hrms.api.get_permitted_fields_for_write', {
      'doctype': doctype,
    });
  ...
  static Future<String> reportsToName(String employee) async =>
      (await _api.call('hrms.api.get_reports_to_employee_name', {
        'employee': employee,
      }))?.toString() ??
      '';
