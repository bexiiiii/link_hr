...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      if (reportsTo != null && reportsTo.isNotEmpty) {
        manager = await Hr.reportsToName(reportsTo).catchError((_) => reportsTo);
      }
      if (mounted) {
        setState(() {
          _employee = doc;
          ...
          child: SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (rows.every((r) => r.$2.trim().isEmpty))
                Text('Данные ещё не заполнены отделом кадров', style: AppText.body.copyWith(color: AppColors.ink3))
              else
                FieldRows(rows: rows),
            ]),
          ),
  ...
  Widget build(BuildContext context) {
    ...
    final cur = v('salary_currency').isEmpty ? s.currency : v('salary_currency');
    final subtitle = [s.designation, s.department].where((x) => x.isNotEmpty).join(' · ');

    final sections = <(IconData, Tone, String, List<(String, String)>)>[
      (CupertinoIcons.person, Tone.violet, 'Сведения о сотруднике', [
        ('ФИО', v('employee_name')),
        ('Табельный номер', v('employee_number').isEmpty ? v('name') : v('employee_number')),
        ('ИИН', v('custom_iin')),
        ('Пол', const {'Male': 'Мужской', 'Female': 'Женский'}[v('gender')] ?? v('gender')),
        ('Дата рождения', Fmt.long(e['date_of_birth'])),
        ('Дата приёма', Fmt.long(e['date_of_joining'])),
        ('Группа крови', v('blood_group')),
      ]),
      (CupertinoIcons.building_2_fill, Tone.green, 'Компания', [
        ('Компания', v('company')),
      ...
      (CupertinoIcons.phone, Tone.amber, 'Контакты', [
        ('Телефон', v('cell_number')),
        ('Личный email', v('personal_email')),
        ('Рабочий email', v('company_email')),
        ('Основной email', v('prefered_email')),
      ]),
      (CupertinoIcons.money_dollar_circle, Tone.dark, 'Зарплата и банк', [
        ('Годовой доход', Fmt.number(e['ctc']) == 0 ? '' : Fmt.money(e['ctc'], cur)),
    ...
    return AppPage(
      header: ScreenHeader(title: 'Профиль', showBack: widget.showBack, actions: [
        CircleButton(
      ...
      body: PageScroll(onRefresh: _load, children: [
        ...
        Center(child: AppAvatar(name: s.fullName, imageUrl: s.image, size: 76)),
        ...
        if (_error != null) ...[ErrorState(error: _error!, onRetry: _load), const SizedBox(height: 16)],
        _Group(children: [
          for (final (icon, tone, title, rows) in sections)
            _Row(
              ...
              tone: tone,
              label: title,
              onTap: _employee == null ? null : () => _showSection(title, rows),
            ),
        ]),
        const SizedBox(height: 14),
        _Group(children: [
        ...
        const SizedBox(height: 24),
        PrimaryButton(label: 'Выйти из аккаунта', kind: ButtonKind.danger, onTap: _logout),
      ]),
    );
  }
}
...
class _Row extends StatelessWidget {
  const _Row({required this.icon, required this.tone, required this.label, this.onTap});
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _toggleLock(bool value) async {
    ...
    if (value) {
      try {
        final ok = await _auth.authenticate(localizedReason: 'Включить вход по Face ID');
        if (!ok) return;
      } catch (_) {
        if (mounted) showToast(context, 'Face ID недоступен на этом устройстве', error: true);
        return;
      }
    }
    ...
    return AppPage(
      ...
      body: PageScroll(children: [
        ...
        SwitchInput(
          ...
          subtitle: _supported ? 'Запрашивать Face ID при открытии приложения' : 'Биометрия недоступна на этом устройстве',
  ...
  Widget build(BuildContext context) {
    return FormScaffold(
      ...
      children: [
        ...
        const Text('Используйте не менее 8 символов: буквы разного регистра, цифры и знаки.', style: AppText.caption),
      ],
    );
  }
}
