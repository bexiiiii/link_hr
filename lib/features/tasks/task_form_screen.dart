...
class _TaskFormScreenState extends State<TaskFormScreen> {
  late final _text = TextEditingController(
      text: widget.task == null ? '' : [widget.task!.title, widget.task!.description].where((s) => s.isNotEmpty).join('\n'));
  ...
  late DueMode _mode;
  DateTime? _other;
  late List<String> _executors = widget.task == null ? [Session.instance.userId] : [widget.task!.allocatedTo];
  ...
  void initState() {
    ...
    if (due == null) {
      ...
      _mode = DueMode.today;
    } else if (!Fmt.dateOnly(due).isAfter(endOfWeek(today)) && widget.initialDue == null) {
      _mode = DueMode.week;
    } else {
      _mode = DueMode.other;
      _other = due;
  ...
  Future<void> _pickMode(DueMode m) async {
    if (m == DueMode.other) {
      final d = await pickDate(context,
          initial: _other ?? DateTime.now().add(const Duration(days: 1)), minimum: DateTime.now());
      if (d == null) return;
      setState(() {
        _mode = m;
        _other = d;
  ...
  Future<void> _pickExecutors() async {
    final picked =
        await pickPeople(context, title: 'Исполнители', multi: !_editing, selected: _executors, frequent: _frequent);
    if (picked != null && picked.isNotEmpty) setState(() => _executors = picked);
  }

  Future<void> _pickReviewer() async {
    final picked = await pickPeople(context,
        title: 'Проверяющий', multi: false, selected: [if (_reviewer.isNotEmpty) _reviewer], frequent: _frequent);
    if (picked != null && picked.isNotEmpty) setState(() => _reviewer = picked.first);
  }
    ...
    try {
      if (_editing) {
        ...
        if (_voice != null) {
          ...
          if (!t.hasVoice) await Api.instance.addTag('ToDo', t.name, TaskTags.voice);
        }
        ...
        Session.instance.notifyDataChanged();
        if (mounted) {
          showToast(context, names.length > 1 ? 'Создано задач: ${names.length}' : 'Задача создана');
          Navigator.pop(context, true);
        }
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    final executors = [for (final id in _executors) People.resolve(_people, id)];
    final reviewer = _reviewer.isEmpty ? null : People.resolve(_people, _reviewer);
    var i = 0;
    return AppPage(
      header: ScreenHeader(title: _editing ? 'Редактирование' : 'Новая задача'),
      bottom: PrimaryButton(label: _editing ? 'Сохранить изменения' : 'Создать задачу', loading: _busy, onTap: _submit),
        ...
        child: PageScroll(children: [
          ...
          Reveal(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              for (final f in _files)
                AttachmentTile(name: f.name, meta: f.meta, onDelete: () => setState(() => _files.remove(f))),
              DashedBox(
                onTap: _attach,
                child: const Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(CupertinoIcons.cloud_upload, size: 20, color: AppColors.ink3),
                  SizedBox(width: 8),
                  Text('Файл или фото', style: TextStyle(fontSize: 15, color: AppColors.ink3)),
                ]),
              ),
            ]),
          ),
                ...
                child: TextField(
                  ...
                  decoration: fieldDecoration(
                    ...
                    suffix: IconButton(
                      ...
                      icon: const Icon(CupertinoIcons.xmark_circle_fill, size: 20, color: AppColors.ink4),
                      onPressed: () => setState(() => _subtasks.removeAt(s).dispose()),
                    ),
                  ),
                ),
              ),
            Align(
              ...
              child: SizedBox(
                ...
                child: DashedBox(
                  height: 42,
                  radius: 10,
                  onTap: () => setState(() => _subtasks.add(TextEditingController())),
                  child: const Row(mainAxisSize: MainAxisSize.min, children: [
                    Text('Добавить подзадачу',
                        style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: AppColors.ink2)),
                    SizedBox(width: 8),
                    Icon(CupertinoIcons.add_circled_solid, size: 20, color: AppColors.chipDot),
                  ]),
                ),
              ),
            ),
                    ...
                    child: Pressable(
                      ...
                      child: AnimatedContainer(
                        ...
                        decoration: BoxDecoration(
                          color: _mode == m ? AppColors.chip : AppColors.surface,
                          borderRadius: BorderRadius.circular(12),
                          border: Border.all(color: _mode == m ? AppColors.chipDot : AppColors.line),
                        ),
                        child: Text(m.label,
                            style: TextStyle(
                              fontSize: 14,
              ...
              AnimatedSize(
                ...
                child: _due == null
                    ...
                    : Padding(
                        ...
                        child: Text('Выполнить до ${Fmt.weekdayDate(_due)}', style: AppText.caption),
                      ),
              ),
            ]),
          ),
              ...
              child: Row(children: [
                Expanded(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text(executors.map((p) => p.userId == Session.instance.userId ? 'Вы' : p.shortName).join(', '),
                        maxLines: 1, overflow: TextOverflow.ellipsis, style: AppText.caption),
                  ]),
                ),
                AvatarStack(people: [for (final p in executors) Person(p.name, p.image)], size: 34),
              ]),
            ),
          ),
          const SizedBox(height: 12),
              ...
              child: Row(children: [
                Expanded(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text(reviewer?.name ?? 'Не назначен', style: AppText.caption),
                  ]),
                ),
                if (reviewer == null)
                  Container(
                    ...
                    child: const Row(mainAxisSize: MainAxisSize.min, children: [
                      Icon(CupertinoIcons.add, size: 18, color: AppColors.ink),
                      SizedBox(width: 6),
                      Text('Добавить', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                    ]),
                  )
                else ...[
                  AppAvatar(name: reviewer.name, imageUrl: reviewer.image, size: 40),
                  IconButton(
                    tooltip: 'Убрать проверяющего',
                    onPressed: () => setState(() => _reviewer = ''),
                    icon: const Icon(CupertinoIcons.xmark_circle_fill, color: AppColors.ink4),
                  ),
                ],
              ]),
            ),
