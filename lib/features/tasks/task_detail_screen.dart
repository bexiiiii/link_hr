...
class _TaskDetailScreenState extends State<TaskDetailScreen> {
  ...
  Future<void> _mutate(Future<void> Function() action, {VoidCallback? rollback, String? success}) async {
    try {
      await action();
  ...
  void _setStatus(TaskStatus status) {
    ...
    _mutate(() => Tasks.setStatus(task.name, status), rollback: () => task.status = previous);
  }

  void _toggle(Subtask sub) {
    setState(() => sub.done = !sub.done);
    _mutate(() => Tasks.toggleSubtask(sub, sub.done), rollback: () => sub.done = !sub.done);
  }
  ...
  Future<void> _addSubtask() async {
    ...
    await _mutate(() async {
      final sub = await Tasks.addSubtask(widget.name, title, allocatedTo: _task!.allocatedTo);
  ...
  Future<void> _reassign() async {
    final options = [
      for (final e in _people.entries) SelectOption(e.key, e.value.$1, subtitle: e.key),
    ];
    final picked = await showSelectSheet(context, title: 'Исполнитель', options: options, selected: _task!.allocatedTo);
    if (picked == null || picked == _task!.allocatedTo) return;
    await _mutate(() => Tasks.setAssignee(widget.name, picked), success: 'Исполнитель изменён');
    _load();
  }
  ...
  Future<void> _setPriority(String priority) async {
    ...
    await _mutate(() => Tasks.update(t.name,
        title: t.title, description: t.description, due: t.due, priority: priority, allocatedTo: t.allocatedTo));
    _load();
  }
  ...
  Future<void> _removeFile(Attachment a) async {
    final ok = await confirmAction(context,
        title: 'Удалить вложение?', message: a.fileName, confirmLabel: 'Удалить', destructive: true);
    if (!ok) return;
    await _mutate(() async {
  ...
  Future<void> _more() async {
    final action = await pickAction(context, actions: const [
    ...
    if (action == 'edit') {
      ...
      final ok = await confirmAction(context,
          ...
          message: 'Задача и её история будут удалены без возможности восстановления.',
  ...
  Widget build(BuildContext context) {
    return AppPage(
      header: ScreenHeader(title: 'Детали задачи', actions: [
        CircleButton(icon: CupertinoIcons.ellipsis, label: 'Действия', onTap: _task == null ? null : _more),
      ]),
      body: _buildBody(),
    );
  }
  ...
  Widget _buildBody() {
    ...
    return GestureDetector(
      ...
      child: PageScroll(onRefresh: _load, padding: const EdgeInsets.fromLTRB(20, 4, 20, 40), children: [
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(
            ...
            child: Icon(CupertinoIcons.bolt_fill, color: taskTone(task.status).solid, size: 24),
          ),
          const SizedBox(width: 14),
          Expanded(child: Text(task.title, style: AppText.display.copyWith(fontSize: 24))),
        ]),
        const SizedBox(height: 14),
        Padding(
          padding: const EdgeInsets.only(left: 66),
          child: Wrap(spacing: 16, runSpacing: 8, children: [
            for (final s in [TaskStatus.completed, TaskStatus.inProgress, TaskStatus.onHold])
              Pressable(
                onTap: () => _setStatus(s),
                ...
                child: Padding(
                  ...
                  child: LegendDot(
                    color: task.status == s ? taskTone(s).solid : AppColors.chipDot,
                    label: s.label,
                    bold: task.status == s,
                  ),
                ),
        ...
        SurfaceCard(
          ...
          child: Divided(children: [
            ...
            DetailRow(
              ...
              value: task.due == null ? 'Без срока' : 'До ${Fmt.weekdayDate(task.due)}',
                      ...
                      child: AnimatedContainer(
                        ...
                        decoration: BoxDecoration(
                          color: task.priority == p
                              ? (p == 'High' ? AppColors.red : AppColors.charcoal)
                        ...
                        child: Text(statusLabel(p),
                            style: TextStyle(
                              ...
                              color: task.priority == p ? Colors.white : AppColors.ink2,
                            )),
                      ),
                    ),
                ]),
            ...
            Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  const Expanded(child: Text('Подзадачи', style: AppText.caption)),
                  if (task.subtasks.isNotEmpty) Text('$done из ${task.subtasks.length}', style: AppText.caption),
                ]),
                ...
                for (final sub in task.subtasks)
                  Dismissible(
                    ...
                    onDismissed: (_) {
                      ...
                      _mutate(() => Tasks.removeSubtask(sub), rollback: () => task.subtasks.add(sub));
                    },
                    child: Pressable(
                      onTap: () => _toggle(sub),
                      scale: 0.99,
                      semanticLabel: '${sub.title}, ${sub.done ? 'выполнено' : 'не выполнено'}',
                      child: Padding(
                          ...
                          Expanded(
                            child: Text(sub.title,
                                style: TextStyle(
                                  ...
                                  color: sub.done ? AppColors.ink3 : AppColors.ink,
                ...
                Row(children: [
                  const Icon(CupertinoIcons.plus, size: 18, color: AppColors.violet),
                  const SizedBox(width: 12),
                  Expanded(
                    child: TextField(
            ...
            Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text(
                  task.description.isEmpty ? 'Описание не добавлено' : task.description,
                  style: AppText.body.copyWith(color: task.description.isEmpty ? AppColors.ink3 : AppColors.ink),
                ),
              ]),
            ),
            Padding(
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: checked ? AppColors.violet : AppColors.chipDot, width: 1.5),
      ),
