...
extension DayMarkStyle on DayMark {
  ...
  IconData get icon => switch (this) {
    DayMark.onTime => AppIcons.clock,
    DayMark.late => AppIcons.timer,
    DayMark.remote => AppIcons.house,
    DayMark.absent => AppIcons.xmarkCircle,
    DayMark.leave => AppIcons.airplane,
    DayMark.dayOff => AppIcons.sunMax,
    _ => AppIcons.circle,
  };
                ...
                Expanded(
                  child: () {
                    ...
                    return _Cell(
                      ...
                      onTap: onDayTap == null || d.mark == DayMark.future
                          ? null
                          : () => onDayTap!(d),
                    );
                  }(),
    ...
    final cell = AnimatedOpacity(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isToday ? AppColors.violet : AppColors.bg,
            width: isToday ? 1.5 : 1,
          ),
        ),
        child: Column(
          ...
          children: [
              ...
              children: [
                Text(
                  ...
                  style: TextStyle(
                    ...
                    color: faded
                        ? AppColors.ink4.withValues(alpha: 0.6)
                        : AppColors.ink,
            ...
            if (m == DayMark.onTime && record!.hours >= 0.1)
              Text(
                '${Fmt.decimal(double.parse(record!.hours.toStringAsFixed(1)))} ч',
                style: const TextStyle(
                  fontSize: 10,
                  fontWeight: FontWeight.w600,
            ...
            FieldRows(
              rows: [
                ...
                (
                  ...
                  d.hours <= 0
                      ? '—'
                      : '${Fmt.decimal(double.parse(d.hours.toStringAsFixed(1)))} ч',
                ),
                if (d.mark == DayMark.late)
                  ('Опоздание', formatLate(d.lateMinutes)),
  ...
  showModalBottomSheet<void>(
    ...
    builder: (c) => DraggableScrollableSheet(
      ...
      builder: (c, controller) => ListView(
        ...
        children: [
          Text(
            'График · ${Fmt.monthYear(sheet.month).toLowerCase()}',
            style: AppText.title,
          ),
          const SizedBox(height: 6),
          Text(
            'Начало рабочего дня ${sheet.workStartLabel}',
          ...
          SurfaceCard(
            ...
            child: Divided(
              children: [
                for (final d in sheet.days.where(
                  (d) => d.mark != DayMark.future,
                ))
                  Padding(
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            d.firstIn == null
                                ? d.mark.label
                                : '${Fmt.time(d.firstIn)} – ${d.lastOut == null ? '…' : Fmt.time(d.lastOut)}',
                            style: AppText.label,
                          ),
                        ),
                        if (d.mark == DayMark.late)
                          ...
                          Icon(
                            d.mark.icon,
                            size: 18,
                            color: d.mark.tone == Tone.neutral
                                ? AppColors.ink4
                                : d.mark.tone.solid,
                          ),
                      ],
...
class _TimesheetGridPanelState extends State<TimesheetGridPanel> {
  ...
  List<MonthSheet> get _sheets =>
      _year == _thisYear ? widget.yearSheets : (_cache[_year] ?? const []);
  ...
  Future<void> _setYear(int y) async {
    ...
    try {
      final sheets = await MonthSheet.loadRange(
        DateTime(y, 1),
        DateTime(y, 12),
      );
  ...
  @override
  Widget build(BuildContext context) {
    final sheets = _sheets.reversed
        .where((s) => _month == null || s.month.month == _month)
        .toList();
    final counts = {
      for (final m in legendMarks)
        m: sheets.fold<int>(0, (s, sh) => s + sh.count(m)),
    };
    return Stack(
      children: [
        CustomScrollView(
          ...
          slivers: [
                    ...
                    Wrap(
                      ...
                      children: [
                        DropdownPill(
                          ...
                          onTap: () async {
                            final picked = await showSelectSheet(
                              ...
                              title: 'Год',
                              options: [
                                for (var y = _thisYear; y >= _thisYear - 4; y--)
                                  SelectOption('$y', '$y'),
                              ],
                              selected: '$_year',
                            );
                        ...
                        DropdownPill(
                          ...
                          label: _month == null
                              ? 'Все'
                              : Fmt.month(DateTime(_year, _month!)),
                          onTap: () async {
                            final last = _year == _thisYear
                                ? DateTime.now().month
                                : 12;
                            final picked = await showSelectSheet(
                              context,
            ...
            if (sheets.isEmpty)
              SliverPadding(
                ...
                sliver: SliverToBoxAdapter(
                  child: _loading
                      ? const Skeleton(height: 360, radius: 20)
                      : const Skeleton(height: 360, radius: 20),
                ),
              )
            else
                      ...
                      children: [
                        Padding(
                          ...
                          child: Row(
                            children: [
                              Text(
                                Fmt.month(sheets[i].month),
                                style: AppText.heading,
                              ),
                              const Spacer(),
                              Text(
                                '${sheets[i].present} дн · ${formatLate(sheets[i].lateMinutes)} опозданий',
                        ...
                        MonthGrid(
                          sheet: sheets[i],
                          filter: _filter,
                          onDayTap: (d) => showDaySheet(
                            context,
                            d,
                            workStart: sheets[i].workStartLabel,
                          ),
                        ),
                      ],
                    ),
