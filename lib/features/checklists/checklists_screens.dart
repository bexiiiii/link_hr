...
class _ChecklistRunScreenState extends State<ChecklistRunScreen> {
  ...
  Widget build(BuildContext context) {
    final run = _run;
    final needsPhoto = run != null && run.meta.photo && _files.isEmpty && !run.closed;
    final canFinish = run != null && !run.closed && run.allMarked && !needsPhoto;
    return AppPage(
      header: ScreenHeader(title: run?.meta.title ?? 'Чеклист'),
      bottom: run == null
          ? null
          : run.closed
              ? const PrimaryButton(label: 'Чеклист завершён', kind: ButtonKind.outline)
              : PrimaryButton(label: 'Завершить', loading: _busy, onTap: canFinish ? _complete : null),
      body: _error != null
          ? PageScroll(children: [ErrorState(error: _error!, onRetry: _load)])
          : run == null
              ? PageScroll(children: const [SkeletonCards(count: 2, height: 160)])
              : PageScroll(onRefresh: _load, children: [
                  if (needsPhoto)
                    Reveal(
                      child: Container(
                        ...
                        child: const Row(children: [
                          Icon(CupertinoIcons.exclamationmark_triangle_fill, color: Color(0xFFD9A21B)),
                          SizedBox(width: 12),
                          Expanded(
                            child: Text('Чеклист нельзя завершить без прикреплённого фото-отчёта',
                                style: TextStyle(fontSize: 13, height: 1.35, color: Color(0xFF6B4E00))),
                          ),
                        ]),
                      ),
                    ),
                        ...
                        child: Row(children: [
                          SizedBox(
                            ...
                            child: CircularProgressIndicator(
                              value: run.items.isEmpty ? 0 : run.items.where((i) => i.state != ItemState.open).length / run.items.length,
                              strokeWidth: 3,
                          ...
                          Expanded(
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Text(run.meta.title, style: AppText.bodyStrong),
                                ...
                                child: Column(children: [
                                  for (final (i, item) in run.items.indexed)
                                    Reveal(
                                      ...
                                      child: Padding(
                                        ...
                                        child: Row(children: [
                                          _Box(
                                            on: item.state == ItemState.done,
                                            icon: Icons.check_rounded,
                                            color: AppColors.green,
                                            label: 'Выполнено',
                                            onTap: run.closed ? null : () => _mark(item, ItemState.done),
                                          ),
                                          const SizedBox(width: 6),
                                          _Box(
                                            on: item.state == ItemState.failed,
                                            icon: Icons.close_rounded,
                                            color: AppColors.charcoal,
                                            label: 'Не выполнено',
                                            onTap: run.closed ? null : () => _mark(item, ItemState.failed),
                                          ),
                                          const SizedBox(width: 12),
                                          Expanded(
                                            child: AnimatedDefaultTextStyle(
                                              ...
                                              style: AppText.body.copyWith(
                                                color: item.state == ItemState.failed ? AppColors.ink3 : AppColors.ink,
                                                decoration: item.state == ItemState.failed ? TextDecoration.lineThrough : null,
                                              ),
                                  ...
                                  if (run.items.isEmpty)
                                    Padding(
                                      ...
                                      child: Text('В чеклисте нет пунктов', style: AppText.caption),
                                    ),
                                ]),
                              ),
                      ),
                  ...
                  for (final f in _files)
                    AttachmentTile(name: f.fileName, meta: f.meta, onTap: () => Files.open(f).catchError((_) {})),
                  if (!run.closed)
                    DashedBox(
                      height: 56,
                      ...
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        _uploading
                            ...
                            : const Icon(CupertinoIcons.cloud_upload_fill, color: AppColors.ink3),
                        const SizedBox(width: 8),
                        Text(_uploading ? 'Загрузка…' : 'Файл или фото', style: const TextStyle(fontSize: 15, color: AppColors.ink3)),
                      ]),
                    ),
                ]),
    );
...
class _Box extends StatelessWidget {
  const _Box({required this.on, required this.icon, required this.color, required this.label, this.onTap});
...
class _ChecklistsScreenState extends State<ChecklistsScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      final r = await Future.wait<Object>([
        ...
        Checklists.runs(from: now.subtract(const Duration(days: 30)), to: now, user: Session.instance.userId),
        People.byUser(),
      ]);
      if (!mounted) return;
      setState(() {
        ...
        _history = (r[2] as List<ChecklistRun>)..sort((a, b) => (b.date ?? now).compareTo(a.date ?? now));
  ...
  Widget build(BuildContext context) {
    return AppPage(
      header: ScreenHeader(title: 'Чеклисты', actions: [
        if (Session.instance.isManager) CircleButton(
          icon: CupertinoIcons.add,
          label: 'Новый чеклист',
          onTap: () async {
            final ok = await pushPage<bool>(context, const ChecklistFormScreen());
            if (ok == true) _load();
          },
        ),
      ]),
      body: Column(children: [
        Padding(
          ...
          child: SegmentTabs(
              labels: Session.instance.isManager ? const ['Сегодня', 'Шаблоны', 'История'] : const ['Сегодня', 'История'],
              index: _tab,
        ...
        Expanded(
          child: PageScroll(onRefresh: _load, children: [
            if (_error != null)
              ...
              ...switch (Session.instance.isManager || _tab == 0 ? _tab : 2) {
                0 => _runList(_today!, empty: 'На сегодня чеклистов нет. Создайте шаблон, и он будет появляться каждый день.'),
                1 => _templateList(),
                _ => _runList(_history!, empty: 'Завершённые чеклисты за 30 дней появятся здесь.', dated: true),
              },
          ]),
        ),
      ]),
  ...
  List<Widget> _runList(List<ChecklistRun> runs, {required String empty, bool dated = false}) {
    if (runs.isEmpty) {
      return [EmptyState(icon: CupertinoIcons.list_bullet, title: 'Пусто', message: empty)];
    }
    return [
      SurfaceCard(
            ...
            Reveal(
              ...
              child: Pressable(
                ...
                child: Padding(
                  ...
                  child: Row(children: [
                    IconBadge(
                      icon: r.closed ? CupertinoIcons.checkmark_alt : CupertinoIcons.list_bullet,
                    ...
                    Expanded(
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Text([if (dated) Fmt.weekdayDate(r.date), r.meta.window, '${r.doneCount}/${r.items.length}'].join(' · '),
    ...
    if (list.isEmpty) {
      return [
        EmptyState(
          ...
          title: 'Шаблонов нет',
          message: 'Шаблон задаёт пункты и время. По нему у сотрудника каждый день появляется чеклист.',
                ...
                child: Padding(
                  ...
                  child: Row(children: [
                    AppAvatar(
                      ...
                      imageUrl: People.resolve(_people, t.allocatedTo).image,
                      size: 42,
                      border: false,
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Text(
                          [People.resolve(_people, t.allocatedTo).shortName, t.meta.window, '${t.itemCount} пунктов', if (t.meta.photo) 'фото'].join(' · '),
                          style: AppText.caption,
                        ),
                      ]),
                    ),
...
class _ChecklistFormScreenState extends State<ChecklistFormScreen> {
        ...
        child: SafeArea(
          ...
          child: Column(children: [
            Row(children: [
              ...
              const Spacer(),
              CupertinoButton(
                onPressed: () => Navigator.pop(c, TimeOfDay(hour: temp.hour, minute: temp.minute)),
            ...
            Expanded(
              child: CupertinoDatePicker(
                ...
                use24hFormat: true,
                minuteInterval: 5,
                initialDateTime: DateTime(2000, 1, 1, temp.hour, temp.minute - temp.minute % 5),
  ...
  Future<void> _submit() async {
    ...
    final items = _items.map((c) => c.text.trim()).where((s) => s.isNotEmpty).toList();
    ...
    try {
      await Checklists.createTemplate(
        meta: ChecklistMeta(title: title, from: _fmt(_from), to: _fmt(_to), photo: _photo),
        items: items,
  ...
  Widget build(BuildContext context) {
    return FormScaffold(
      ...
      onSubmit: _submit,
      children: [
        AppTextField(label: 'Название', controller: _title, required: true, hint: 'Например, открытие смены'),
        const FormGap(),
            ...
            Expanded(
              child: SurfaceCard(
                ...
                child: Row(children: [
                  ...
                  const Spacer(),
                  Text(_fmt(value) ?? '—', style: AppText.number.copyWith(fontSize: 15)),
                ]),
              ),
            ),
        ...
        for (var i = 0; i < _items.length; i++)
          Padding(
            ...
            child: TextField(
              ...
              onChanged: (v) {
                if (i == _items.length - 1 && v.isNotEmpty) setState(() => _items.add(TextEditingController()));
              },
        ...
        SurfaceCard(
          onTap: () async {
            final picked = await pickPeople(context, title: 'Кому назначить', multi: true, selected: _assignees);
            if (picked != null && picked.isNotEmpty) setState(() => _assignees = picked);
          },
          ...
          child: Row(children: [
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text(_assignees.map((id) => People.resolve(_people, id).shortName).join(', '),
                    maxLines: 1, overflow: TextOverflow.ellipsis, style: AppText.caption),
              ]),
            ),
            AvatarStack(people: [
              for (final id in _assignees) Person(People.resolve(_people, id).name, People.resolve(_people, id).image),
            ]),
          ]),
        ),
      ],
