...
const _cardViolet = Color(0xFFF7D9FF);

class AchievementCard extends StatelessWidget {
  const AchievementCard({super.key, required this.achievement, this.compact = false});
  ...
  Widget build(BuildContext context) {
    ...
    return SurfaceCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(a.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: AppText.bodyStrong.copyWith(fontSize: 13)),
        const Spacer(),
        ClipRRect(
        ...
        Text(a.unlocked ? 'Получено ${Fmt.date(a.unlockedAt)}' : '${a.progress} из ${a.target}', style: AppText.caption),
      ]),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: unlocked ? AppColors.plum : AppColors.chip, width: size * 0.07),
      ),
      child: Icon(achievementIcon(id), size: size * 0.44, color: unlocked ? AppColors.plum : AppColors.ink4),
    );
  }
}
...
Future<void> showAchievementDialog(BuildContext context, Achievement a) {
  return showGeneralDialog<void>(
    ...
    transitionBuilder: (_, anim, _, child) {
      ...
      return FadeTransition(opacity: t, child: ScaleTransition(scale: Tween(begin: 0.9, end: 1.0).animate(t), child: child));
    },
          ...
          child: Stack(children: [
            ...
            Padding(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  child: Reveal(
                    ...
                    child: SizedBox(
                      ...
                      child: Stack(alignment: Alignment.center, children: [
                        Icon(Icons.emoji_events_rounded, size: 170, color: AppColors.amber.withValues(alpha: 0.22)),
                ...
                Text('Достижение «${a.title}» получено', style: AppText.title),
                const SizedBox(height: 8),
                Text('Поздравляем! ${a.description}', style: AppText.body.copyWith(color: AppColors.ink2)),
                const SizedBox(height: 22),
                PrimaryButton(label: 'Ознакомлен', onTap: () => Navigator.pop(c)),
              ]),
            ),
          ]),
        ),
...
class _AchievementsScreenState extends State<AchievementsScreen> {
  late List<Achievement>? _items = widget.initial == null || widget.initial!.isEmpty ? null : widget.initial;
  Object? _error;
  ...
  Future<void> _load() async {
    try {
      ...
      if (mounted) {
        setState(() => _items = Achievements.compute(
            sheets: r[0] as List<MonthSheet>, tasks: r[1] as List<TaskItem>, runs: r[2] as List<ChecklistRun>, me: me));
      }
    } catch (e) {
                  ...
                  slivers: [
                    ...
                    SliverPadding(
                      ...
                      sliver: SliverToBoxAdapter(
                        child: Text('Получено $unlocked из ${items.length}', style: AppText.label.copyWith(color: AppColors.ink3)),
                      ),
                    ),
                    SliverPadding(
                      ...
                      sliver: SliverGrid.builder(
                        ...
                        itemBuilder: (_, i) => Reveal(index: i, scale: true, child: AchievementCard(achievement: items[i])),
                      ),
                    ),
                  ],
                ),
    ...
    return AppPage(
      ...
      body: Stack(children: [
        PageScroll(children: [
            ...
            child: Reveal(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: a.unlocked ? _cardViolet : AppColors.surface,
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(color: a.unlocked ? AppColors.plum.withValues(alpha: 0.4) : AppColors.line, width: 2),
                ),
                child: Column(children: [
                  Text(a.title.toUpperCase(),
                      textAlign: TextAlign.center,
                      style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, letterSpacing: 0.4, color: AppColors.plum)),
                  const SizedBox(height: 22),
                  ...
                  AnimatedNumber(
                    value: percent,
                    suffix: '%',
                    style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.plum),
                  ),
                  const SizedBox(height: 8),
                  ClipRRect(
                    borderRadius: BorderRadius.circular(3),
          ...
          if (a.unlocked) Text(Fmt.long(a.unlockedAt), textAlign: TextAlign.center, style: AppText.label.copyWith(color: AppColors.ink3)),
          const SizedBox(height: 6),
          Text(
            a.unlocked ? 'Достижение разблокировано' : 'Осталось ${a.target - a.progress} до цели',
            textAlign: TextAlign.center,
            style: AppText.display.copyWith(fontSize: 22),
          ),
          const SizedBox(height: 10),
          Text(a.description, textAlign: TextAlign.center, style: AppText.body.copyWith(color: AppColors.ink2)),
            ...
            Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              CircleButton(
                ...
                onTap: () => SharePlus.instance.share(ShareParams(
                  text: 'Я получил(а) достижение «${a.title}» в Link: ${a.description}',
                )),
              ),
              ...
              CircleButton(
                ...
                foreground: const Color(0xFFD6249F),
                onTap: () => SharePlus.instance.share(ShareParams(
                  text: 'Достижение «${a.title}» в Link',
                )),
              ),
            ]),
        ]),
