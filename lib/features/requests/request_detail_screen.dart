...
Future<void> persistRequest(
  ...
  if (original == null) {
    ...
    Navigator.pushReplacement(
      ...
      CupertinoPageRoute(
        builder: (_) =>
            RequestDetailScreen(kind: kind, name: saved['name'].toString()),
      ),
    );
  } else {
...
class _RequestDetailScreenState extends State<RequestDetailScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      if (workflow.isNotEmpty && Fmt.number(doc['docstatus']) != 2) {
        transitions = await Hr.workflowTransitions(
          doc,
        ).catchError((_) => <String>[]);
      }
      if (!mounted) return;
  ...
  String get _status {
    ...
    final overrides =
        _workflow['override_status'] == 1 ||
        _workflow['override_status'] == true;
  ...
  Future<void> _cancel() async {
    ...
    await _run(
      'cancel',
      () => _api.cancel(kind.doctype, widget.name),
      'Документ отменён',
    );
  }
  ...
  Future<void> _more() async {
    ...
    final pending =
        kind.approvalField == null ||
        doc[kind.approvalField] == kind.pendingStatus;
    ...
    final action = await pickAction(
      ...
      actions: [
        ...
        if (canCancel)
          const SheetAction('cancel', 'Отменить документ', destructive: true),
        if (canDelete)
          const SheetAction('delete', 'Удалить', destructive: true),
      ],
    );
  ...
  List<(String, String)> _rows(Json d) {
    ...
    return switch (kind) {
      RequestKind.leave => [
        ...
        (
          'Полдня',
          yes('half_day').isEmpty
              ? ''
              : Fmt.long(d['half_day_date'] ?? d['from_date']),
        ),
        ('Количество дней', Fmt.days(d['total_leave_days'])),
        (
          'Остаток на дату подачи',
          d['leave_balance'] == null ? '' : Fmt.days(d['leave_balance']),
        ),
        (
          'Согласующий',
          text('leave_approver_name').isEmpty
              ? text('leave_approver')
              : text('leave_approver_name'),
        ),
      ...
      RequestKind.advance => [
        ...
        (
          'Удержать остаток из зарплаты',
          yes('repay_unclaimed_amount_from_salary'),
        ),
      ],
      RequestKind.shift => [
        ('Сотрудник', text('employee_name')),
  ...
  Widget? _actions() {
        ...
        children: [
          for (final t in _transitions) ...[
            PrimaryButton(
              ...
              loading: _busy == t,
              kind:
                  t.toLowerCase().contains('reject') ||
                      t.toLowerCase().contains('cancel')
                  ? ButtonKind.danger
              ...
              onTap: () => _run(
                t,
                () => Hr.applyWorkflow(doc, t),
                'Действие «$t» выполнено',
              ),
            ),
    ...
    final isApprover =
        kind.approverField != null &&
        doc[kind.approverField] == _session.userId;
    final selfBlocked =
        kind == RequestKind.leave &&
        _isOwn &&
        _session.preventSelfLeaveApproval;
    final canApprove =
        pending &&
        _writable.contains(field) &&
        (isApprover || !_isOwn) &&
        !selfBlocked;
    final canSubmit =
        (_perms['submit'] ?? false) &&
        (field == null || !pending) &&
        (!_isOwn || isApprover || field == null);
  ...
  Widget _body() {
    ...
    final advances = (doc['advances'] as List? ?? const []).cast<Map>().where(
      (a) => Fmt.number(a['allocated_amount']) > 0,
    );
                      ...
                      children: [
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                e['expense_type']?.toString() ?? '',
                                style: AppText.bodyStrong,
                              ),
                              const SizedBox(height: 2),
                              Text(
                                [
                                  Fmt.dayMonth(e['expense_date']),
                                  stripHtml(e['description']?.toString()),
                        ...
                        Column(
                          ...
                          children: [
                            Text(
                              Fmt.money(e['amount'], cur),
                              style: AppText.number,
                            ),
                            if (Fmt.number(e['sanctioned_amount']) !=
                                Fmt.number(e['amount']))
                              Text(
                                'утв. ${Fmt.money(e['sanctioned_amount'], cur)}',
                                style: AppText.caption,
                              ),
                          ],
                        ),
          ...
          FieldRows(
            rows: [
              for (final a in advances)
                (
                  a['employee_advance'].toString(),
                  Fmt.money(a['allocated_amount'], cur),
                ),
            ],
          ),
        ],
