...
enum RequestKind {
  ...
  Future<List<Json>> fetch({bool team = false, int? limit}) => switch (this) {
    ...
    RequestKind.advance =>
      team ? Future.value(<Json>[]) : Hr.advances(limit: limit),
  ...
  String status(Json d) {
    ...
    if (wf != null && wf.isNotEmpty && (d[wf]?.toString() ?? '').isNotEmpty)
      return d[wf].toString();
    final docstatus = Fmt.number(d['docstatus']).toInt();
    if (docstatus == 2) return 'Cancelled';
    return switch (this) {
      RequestKind.leave ||
      RequestKind.shift => d['status']?.toString() ?? 'Open',
      RequestKind.expense => d['approval_status']?.toString() ?? 'Draft',
      RequestKind.advance =>
        docstatus == 0 ? 'Draft' : (d['status']?.toString() ?? 'Submitted'),
  ...
  String heading(Json d) => switch (this) {
    ...
    RequestKind.advance => stripHtml(
      d['purpose']?.toString(),
    ).split('\n').first,
    RequestKind.shift => d['shift_type']?.toString() ?? singular,
    RequestKind.attendance => statusLabel(
      d['reason']?.toString() ?? 'Work From Home',
    ),
  };
  ...
  String subtitle(Json d) {
    ...
    return switch (this) {
      RequestKind.leave =>
        '${Fmt.range(d['from_date'], d['to_date'])} · ${Fmt.days(d['total_leave_days'])}',
      RequestKind.expense =>
        '${Fmt.money(d['total_claimed_amount'], currency)} · ${Fmt.dayMonth(d['posting_date'])}',
      RequestKind.advance =>
        '${Fmt.money(d['advance_amount'], currency)} · ${Fmt.dayMonth(d['posting_date'])}',
...
Future<List<(RequestKind, Json)>> fetchRequestFeed({
  required bool team,
  int perKind = 10,
}) async {
  final kinds = team
      ? RequestKind.values.where((k) => k.hasTeam).toList()
      : RequestKind.values;
  final results = await Future.wait(
    kinds.map(
      (k) => k
          .fetch(team: team, limit: perKind)
          .then(
  ...
  final all = results.expand((e) => e).toList()
    ..sort(
      (a, b) => (Fmt.parse(b.$2['creation']) ?? DateTime(2000)).compareTo(
        Fmt.parse(a.$2['creation']) ?? DateTime(2000),
      ),
    );
    ...
    return Padding(
      ...
      child: SurfaceCard(
        ...
        onTap: () async {
          await pushPage(
            context,
            RequestDetailScreen(kind: kind, name: data['name'].toString()),
          );
          onChanged?.call();
        },
        child: Row(
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  if (showEmployee &&
                      (data['employee_name'] ?? '').toString().isNotEmpty) ...[
                    Text(
