...
class _RequestListScreenState extends State<RequestListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final visible = _status == null ? _items : _items.where((d) => kind.status(d) == _status).toList();

    return AppPage(
      header: ScreenHeader(title: kind.plural, actions: [
        CircleButton(icon: CupertinoIcons.add, label: 'Новая заявка', onTap: () => pushPage(context, kind.form())),
      ]),
              ...
              children: [
                for (final e in counts.entries) ...[
                  CountChip(
                    ...
                    count: e.value,
                    tone: toneForStatus(e.key) == Tone.neutral ? Tone.dark : toneForStatus(e.key),
                    selected: _status == e.key,
                    onTap: () => setState(() => _status = _status == e.key ? null : e.key),
                  ),
        ...
        Expanded(
          child: PageScroll(onRefresh: _load, children: [
            if (_loading)
              ...
              EmptyState(
                icon: kind.icon,
                title: _tab == 0 ? 'Заявок пока нет' : 'Нечего согласовывать',
                message: _tab == 0
                ...
                onAction: _tab == 0 ? () => pushPage(context, kind.form()) : null,
              )
            else
              for (final d in visible) RequestTile(kind: kind, data: d, showEmployee: _tab == 1, onChanged: _load),
          ]),
        ),
      ]),
    );
