...
class SummarySection extends StatelessWidget {
  const SummarySection({super.key, required this.title, this.onSeeAll, this.seeAllLabel = 'Все', this.top = 28});
        ...
        if (onSeeAll != null)
          Pressable(
            ...
            child: Padding(
              ...
              child: Text(seeAllLabel, style: AppText.label.copyWith(color: AppColors.green, fontWeight: FontWeight.w500)),
            ),
          ),
      ]),
    );
...
class StatTile extends StatelessWidget {
  const StatTile({super.key, required this.value, required this.label, required this.icon, this.unit, this.muted = false});
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Expanded(
            child: Text.rich(
              TextSpan(children: [
                TextSpan(text: value),
                if (unit != null) TextSpan(text: ' $unit', style: AppText.label.copyWith(color: AppColors.ink3)),
              ]),
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
              style: AppText.clock.copyWith(fontSize: 24, color: muted ? AppColors.ink4 : AppColors.ink),
            ),
          ),
          Icon(icon, size: 20, color: AppColors.ink3),
        ]),
        const SizedBox(height: 6),
        Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: AppText.label.copyWith(color: AppColors.ink3)),
      ]),
    );
  }
}
...
class ClockCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    Widget cell(String label, DateTime? t) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(label, style: AppText.label.copyWith(color: AppColors.ink3)),
          const SizedBox(height: 4),
          Text(t == null ? '– –' : Fmt.time(t), style: AppText.clock.copyWith(fontSize: 20)),
        ]);
    return SurfaceCard(
      ...
      child: Row(children: [
        ...
        if (actionLabel != null)
          PrimaryButton(label: actionLabel!, onTap: onAction, height: 44, expand: false),
      ]),
    );
  }
}
...
class RequestCounters extends StatelessWidget {
  const RequestCounters({super.key, required this.total, required this.approved, required this.declined, this.onTap, this.loading = false});
  ...
  Widget build(BuildContext context) {
          ...
          child: SurfaceCard(
            ...
            child: Column(children: [
              loading
                  ...
                  : Text('$value', style: AppText.clock.copyWith(fontSize: 22, color: color)),
              const SizedBox(height: 4),
              Text(label, style: AppText.label.copyWith(color: AppColors.ink3)),
            ]),
          ),
    ...
    return Row(children: [
      ...
      const SizedBox(width: 10),
      cell(declined, 'Отклонено', declined > 0 ? AppColors.red : AppColors.green),
    ]);
  }
}
    ...
    return SurfaceCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const Spacer(),
          Flexible(child: StatusPill(null, label: pill, tone: pillTone)),
        ]),
        const SizedBox(height: 12),
        Text(title, maxLines: 2, overflow: TextOverflow.ellipsis, style: AppText.cardTitle.copyWith(fontSize: 15)),
        if (subtitle != null) ...[
          const SizedBox(height: 6),
          Text(subtitle!, maxLines: 2, overflow: TextOverflow.ellipsis, style: AppText.label.copyWith(color: AppColors.ink3)),
        ],
        if (progress != null) ...[
          const SizedBox(height: 12),
          ClipRRect(
...
class CheckinLogTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (isIn && time != null && workStart != null) {
      final due = DateTime(time.year, time.month, time.day, workStart!.$1, workStart!.$2);
    ...
    return Padding(
      ...
      child: Row(children: [
        Container(
          width: 44,
          height: 44,
          decoration: BoxDecoration(color: isIn ? AppColors.greenSoft : AppColors.chip, shape: BoxShape.circle),
          child: Icon(
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(time == null ? '' : Fmt.long(time), style: AppText.caption),
          ]),
        ),
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Text(time == null ? '' : Fmt.time(time), style: AppText.number.copyWith(fontSize: 16)),
          if (note != null) Text(note, style: AppText.caption.copyWith(color: noteColor)),
        ]),
      ]),
    );
  }
