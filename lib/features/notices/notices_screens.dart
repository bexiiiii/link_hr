...
Future<void> showNoticeDialog(
  ...
  return showGeneralDialog<void>(
    ...
    pageBuilder: (c, _, _) =>
        _NoticeCard(notice: n, from: People.resolve(people, n.from)),
  );
}
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${Fmt.date(n.created)} ${Fmt.time(n.created)} · ${widget.from.name}',
...
class _NoticesScreenState extends State<NoticesScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _items = (r[0] as List<Notice>)
              .where((n) => n.date == null || !n.date!.isAfter(DateTime.now()))
              .toList();
      ...
      body: PageScroll(
        ...
        children: [
          ...
          if (_error != null)
            ...
            const EmptyState(
              icon: AppIcons.speaker2,
              title: 'Оповещений нет',
              message:
                  'Объявления руководителей и HR появятся здесь и всплывут на главном экране.',
            )
          else
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                '${People.resolve(_people, n.from).name} · ${Fmt.relative(n.created)}',
                                style: AppText.caption,
                              ),
                            ],
                          ),
...
class _NoticeFormScreenState extends State<NoticeFormScreen> {
    ...
    People.all()
        .then((p) {
          if (mounted) {
            setState(() {
              _people = p
                  .where((x) => x.userId != Session.instance.userId)
                  .toList();
  ...
  Future<void> _send() async {
    ...
    try {
      final n = await Notices.send(
        title: title,
        body: _body.text,
        recipients: _picked.toList(),
        when: _when,
        files: _files,
      );
      Session.instance.notifyDataChanged();
      if (mounted) {
        showToast(
          context,
          'Оповещение отправлено: $n ${Fmt.plural(n, 'получатель', 'получателя', 'получателей')}',
        );
  ...
  Widget build(BuildContext context) {
    final all =
        _people.isNotEmpty && _people.every((p) => _picked.contains(p.userId));
    final byDepartment = <String, List<PersonInfo>>{};
    for (final p in _people) {
      byDepartment
          .putIfAbsent(
            p.department.isEmpty ? 'Без отдела' : p.department,
            () => [],
          )
          .add(p);
    }
    ...
    return AppPage(
      ...
      body: GestureDetector(
        ...
        child: PageScroll(
          children: [
            Text(
              'Отправьте оповещение в виде карточки и уведомления',
            ...
            Padding(
              ...
              child: Text(
                'Ограничение от 5 до 50 символов · $len',
                style: AppText.caption.copyWith(
                  color: len > 0 && len < 5 ? AppColors.red : AppColors.ink3,
                ),
              ),
            ),
            const FormGap(),
            ...
            TextField(
              ...
              decoration: fieldDecoration(
                hint: 'Введите описание и детали оповещения',
              ),
            ),
            Padding(
              padding: const EdgeInsets.only(top: 6, left: 2),
            ...
            SurfaceCard(
              ...
              child: Row(
                children: [
                  Expanded(
                    child: Text(
                      '${Fmt.long(_when)}, ${Fmt.time(_when)}',
                      style: AppText.body,
                    ),
                  ),
                  const Icon(AppIcons.calendar, color: AppColors.ink),
                ],
              ),
            ),
            const SizedBox(height: 22),
            const Text(
              'Часто взаимодействующие сотрудники',
                  ...
                  children: [
                    for (final (i, p) in frequent.indexed)
                      Reveal(
                        ...
                        child: Pressable(
                          onTap: () => setState(
                            () => _picked.contains(p.userId)
                                ? _picked.remove(p.userId)
                                : _picked.add(p.userId),
                          ),
                              ...
                              children: [
                                AnimatedContainer(
                                  ...
                                  decoration: BoxDecoration(
                                    shape: BoxShape.circle,
                                    border: Border.all(
                                      color: _picked.contains(p.userId)
                                          ? AppColors.violet
                                          : Colors.transparent,
                                      width: 2,
                                    ),
            ...
            _CheckRow(
              ...
              onTap: () => setState(
                () => all
                    ? _picked.clear()
                    : _picked.addAll(_people.map((p) => p.userId)),
              ),
            ),
            for (final entry in byDepartment.entries) ...[
              _CheckRow(
                value: entry.value.every((p) => _picked.contains(p.userId))
                    ? true
                    : (entry.value.any((p) => _picked.contains(p.userId))
                          ? null
                          : false),
                title: entry.key,
                subtitle:
                    '${entry.value.length} ${Fmt.plural(entry.value.length, 'сотрудник', 'сотрудника', 'сотрудников')}',
                expanded: _openDepartments.contains(entry.key),
                onExpand: () => setState(
                  () => _openDepartments.contains(entry.key)
                      ? _openDepartments.remove(entry.key)
                      : _openDepartments.add(entry.key),
                ),
                onTap: () => setState(() {
                  final allIn = entry.value.every(
                    (p) => _picked.contains(p.userId),
                  );
