...
class _SalarySlipDetailScreenState extends State<SalarySlipDetailScreen> {
  ...
  Widget build(BuildContext context) {
    final d = _doc;
    return AppPage(
      header: ScreenHeader(title: 'Расчётный листок', actions: [
        _downloading
            ? const SizedBox(width: 48, height: 48, child: CupertinoActivityIndicator())
            : CircleButton(icon: CupertinoIcons.arrow_down_doc, label: 'Скачать PDF', onTap: d == null ? null : _download),
      ]),
      body: _error != null
          ? PageScroll(children: [ErrorState(error: _error!, onRetry: _load)])
          : d == null
              ? PageScroll(children: const [
  ...
  Widget _content(Json d) {
    ...
    return PageScroll(onRefresh: _load, children: [
      Text(Fmt.monthYear(Fmt.parse(d['start_date']) ?? DateTime.now()), style: AppText.display),
      const SizedBox(height: 6),
      Row(children: [
        Text(Fmt.range(d['start_date'], d['end_date']), style: AppText.label.copyWith(color: AppColors.ink3)),
      ...
      SurfaceCard(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(Fmt.money(d['rounded_total'] ?? d['net_pay'], cur),
              style: AppText.display.copyWith(fontSize: 28, fontFeatures: const [FontFeature.tabularFigures()])),
          ...
          Row(children: [
            Expanded(child: _Figure(label: 'Начислено', value: Fmt.money(d['gross_pay'], cur), color: AppColors.greenDeep)),
            Expanded(child: _Figure(label: 'Удержано', value: Fmt.money(d['total_deduction'], cur), color: AppColors.red)),
          ]),
        ]),
      ),
      if (earnings.isNotEmpty) ...[const SectionHeader('Начисления'), FieldRows(rows: earnings)],
      if (deductions.isNotEmpty) ...[const SectionHeader('Удержания и налоги'), FieldRows(rows: deductions)],
      ...
      FieldRows(rows: [
        ...
        ('Без сохранения зарплаты', Fmt.number(d['leave_without_pay']) == 0 ? '' : Fmt.decimal(d['leave_without_pay'])),
        ('Отсутствие', Fmt.number(d['absent_days']) == 0 ? '' : Fmt.decimal(d['absent_days'])),
