...
class _FinanceScreenState extends State<FinanceScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final rejected =
        Fmt.number(_summary['total_rejected_amount']) +
        (Fmt.number(_summary['total_claimed_in_approved']) - approved);
    final total =
        pending +
        Fmt.number(_summary['total_claimed_in_approved']) +
        Fmt.number(_summary['total_rejected_amount']);
          ...
          CircleButton(
            ...
            onTap: () async {
              ...
              if (kind != null && context.mounted)
                pushPage(context, RequestKind.values.byName(kind).form());
            },
          ),
        ],
      ),
      body: PageScroll(
        ...
        children: [
            ...
            SurfaceCard(
              ...
              onTap: () => pushPage(
                ...
                _slip == null
                    ...
                    : SalarySlipDetailScreen(name: _slip!['name'].toString()),
              ),
              child: Column(
          ...
          Row(
            children: [
              ...
              Expanded(
                child: PrimaryButton(
                  label: 'Аванс',
                  icon: AppIcons.moneyDollarCircle,
                  height: 52,
          ...
          SectionHeader(
            ...
            onAction: () => pushPage(
              context,
              const RequestListScreen(kind: RequestKind.expense),
            ),
          ),
          if (_loading)
            ...
            const EmptyState(
              ...
              message:
                  'Добавьте расходы на командировку, связь или питание и отправьте их на согласование.',
            )
          else
            for (final c in _claims)
              RequestTile(kind: RequestKind.expense, data: c, onChanged: _load),
          SectionHeader(
            'Баланс авансов',
            actionLabel: 'Все',
            onAction: () => pushPage(
              context,
              const RequestListScreen(kind: RequestKind.advance),
            ),
          ),
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  Text(
                                    stripHtml(a['purpose']?.toString()),
                                    maxLines: 1,
                                    overflow: TextOverflow.ellipsis,
                                    style: AppText.bodyStrong,
                                  ),
                                  Text(
                                    '${Fmt.dayMonth(a['posting_date'])} · ${statusLabel(a['status']?.toString())}',
