...
Future<SelfieResult> takeSelfie(BuildContext context, {required String actionLabel}) async {
  final result = await Navigator.of(context, rootNavigator: true).push<SelfieResult>(
    PageRouteBuilder(
      opaque: true,
...
class _SelfieScreenState extends State<_SelfieScreen> with WidgetsBindingObserver {
  ...
  Future<void> _open() async {
    try {
      ...
      if (cameras.isEmpty) {
        if (mounted) Navigator.pop(context, const SelfieResult(SelfieOutcome.unavailable));
        return;
      }
      ...
      if (!mounted) return;
      if (e.code.contains('AccessDenied') || e.code.contains('AccessRestricted')) {
  ...
  Future<void> _capture() async {
    ...
    try {
      ...
      if (mounted) showToast(context, 'Не удалось сделать снимок. Попробуйте ещё раз.', error: true);
    } finally {
    ...
    return AnnotatedRegion<SystemUiOverlayStyle>(
      ...
      child: Scaffold(
        ...
        body: Stack(fit: StackFit.expand, children: [
          ...
          if (_problem == 'denied') _DeniedNotice(onClose: () => Navigator.pop(context, const SelfieResult(SelfieOutcome.denied))),
            ...
            child: Stack(children: [
              Positioned(
                ...
                child: CircleButton(
                  ...
                  size: 48,
                  iconSize: 19,
                  onTap: () => Navigator.pop(context, const SelfieResult(SelfieOutcome.cancelled)),
                ),
              ),
              if (_problem == null)
                Positioned(
                          ...
                          child: Pressable(
                            ...
                            child: Container(
                              ...
                              child: _capturing
                                  ...
                                  : const Icon(CupertinoIcons.viewfinder, color: Colors.white, size: 32),
                            ),
                          ),
                        )
                      : Row(children: [
                          ...
                          Expanded(
                            child: PrimaryButton(
                              ...
                              onTap: () => Navigator.pop(context, SelfieResult(SelfieOutcome.taken, _shot)),
                            ),
                          ),
                        ]),
                ),
  ...
  Widget build(BuildContext context) {
    return Pressable(
      ...
      child: Container(
        ...
        child: Text(label, style: AppText.bodyStrong.copyWith(color: Colors.white)),
      ),
    );
  }
}
    ...
    return Center(
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text('Нет доступа к камере', style: AppText.heading.copyWith(color: Colors.white)),
