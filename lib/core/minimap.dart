  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: LayoutBuilder(builder: (context, c) {
        ...
        final tiles = <Widget>[];
        for (var tx = (left / tile).floor(); tx <= ((left + w) / tile).floor(); tx++) {
          for (var ty = (top / tile).floor(); ty <= ((top + height) / tile).floor(); ty++) {
            ...
            tiles.add(Positioned(
              ...
              child: Image.network(
                ...
                errorBuilder: (_, _, _) => const ColoredBox(color: Color(0xFFE3ECF2)),
              ),
            ));
          }
        }
        return ClipRect(
          child: Stack(children: [
            const Positioned.fill(child: ColoredBox(color: Color(0xFFDCE8F0))),
            ...tiles,
            Center(
              child: Transform.translate(
                offset: const Offset(0, -16),
                child: Icon(
                  located ? CupertinoIcons.location_solid : CupertinoIcons.location,
                  size: 36,
                  color: AppColors.red,
                ),
              ),
            ...
            Positioned(
              ...
              child: Container(
                ...
                child: const Text('© OpenStreetMap', style: TextStyle(fontSize: 10, color: AppColors.ink2)),
              ),
            ),
          ]),
        );
