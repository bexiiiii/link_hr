...
class UnderlineTabs extends StatelessWidget {
  const UnderlineTabs({super.key, required this.labels, required this.index, required this.onChanged});
                  ...
                  child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                    ...
                    AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        color: i == index ? AppColors.charcoal : Colors.transparent,
...
class DropdownPill extends StatelessWidget {
  const DropdownPill({super.key, required this.label, required this.onTap, this.prefix});
  ...
  Widget build(BuildContext context) {
    return Row(mainAxisSize: MainAxisSize.min, children: [
      if (prefix != null) ...[Text(prefix!, style: AppText.label), const SizedBox(width: 8)],
      Pressable(
        onTap: onTap,
        ...
        child: Container(
          ...
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            const Icon(CupertinoIcons.chevron_down, size: 13, color: AppColors.ink2),
          ]),
        ),
      ),
    ]);
...
class MonthYearPicker extends StatelessWidget {
  const MonthYearPicker({super.key, required this.month, required this.onChanged});
  ...
  Widget build(BuildContext context) {
    ...
    return Wrap(spacing: 18, runSpacing: 10, crossAxisAlignment: WrapCrossAlignment.center, children: [
      DropdownPill(
        prefix: 'Год',
        label: '${month.year}',
        onTap: () async {
          final picked = await showSelectSheet(context,
              title: 'Год',
              options: [for (var y = now.year; y >= now.year - 4; y--) SelectOption('$y', '$y')],
              selected: '${month.year}');
      ...
      DropdownPill(
        ...
        onTap: () async {
          ...
          final picked = await showSelectSheet(context,
              title: 'Месяц',
              options: [for (var m = last; m >= 1; m--) SelectOption('$m', Fmt.month(DateTime(month.year, m)))],
              selected: '${month.month}');
          if (picked != null) onChanged(DateTime(month.year, int.parse(picked)));
        },
      ),
    ]);
  }
...
class TagChip extends StatelessWidget {
  const TagChip({super.key, required this.label, required this.tone, this.icon, this.selected = false, this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    final chip = AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: selected ? color.withValues(alpha: 0.35) : AppColors.line),
      ),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
    ...
    return onTap == null ? chip : Pressable(onTap: onTap, semanticLabel: label, child: chip);
  }
}
          ...
          Expanded(
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(items[i].$1, style: AppText.label.copyWith(color: AppColors.ink3)),
...
class GroupLabel extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) =>
      Padding(padding: const EdgeInsets.fromLTRB(4, 24, 4, 10), child: Text(text, style: AppText.label));
}
